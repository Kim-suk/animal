...
<body>
		...
		<div class="animal-photo">
			<c:choose>
				<c:when test="${product.animal_type == '강아지'}">
					<img
						src="https://image.store.bemypet.kr/content/uploads/2021/05/03104401/%EC%8D%B8%EB%84%A4%EC%9D%BC-%EB%A6%AC%EB%A6%AC-2.png"
						alt="강아지 이미지">
				</c:when>
				<c:when test="${product.animal_type == '고양이'}">
					<img
						src="https://image.store.bemypet.kr/content/uploads/2021/05/03104459/%EC%8D%B8%EB%84%A4%EC%9D%BC-%EC%82%BC%EC%83%89-2-600x600.png"
						alt="고양이 이미지">
				</c:when>
			</c:choose>
			...
			<button
				style="margin-top: 20px; padding: 10px 20px; font-size: 16px;">
				<a href="${product.link }">링크</a>
			</button>

			<!-- 장바구니 담기 버튼 -->
			<form id="cartForm" action="/animal/cart/add" method="post"
				style="margin-top: 20px;">
				<input type="hidden" name="productId" value="${product.id}" /> <input
					type="hidden" name="productName" value="${product.name}" /> <input
					type="hidden" name="price" value="${product.price}" /> <input
					type="hidden" name="imageUrl" value="${product.image_url}" /> <input
					type="hidden" name="quantity" value="1" />
				<button type="button" onclick="checkLoginBeforeAddToCart()"
					style="padding: 10px 20px; font-size: 16px;">장바구니에 담기</button>
			</form>

			<!-- 장바구니 보기 버튼 -->
			<form action="/animal/cart/list" method="get"
				style="margin-top: 10px;">
				<button type="submit" style="padding: 10px 20px; font-size: 16px;">장바구니
					보기</button>
			</form>
		</div>
	</div>
    ...
    button.addEventListener("click", () => {
         ...
         const loginId = "<c:out value='${sessionScope.loginId}'/>";
        
        if (!loginId || loginId.trim() === "" || loginId === "null") {
            alert("로그인이 필요합니다.");
        ...
        tossPayments.requestPayment("카드", {
            ...
            successUrl: "http://localhost:8080/animal/payment/success.do?productName=" + encodeURIComponent("${product.name}") + "&productImage=" + encodeURIComponent("${product.image_url}"),
